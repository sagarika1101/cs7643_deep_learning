...
class CaptioningSolver(object):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.callback_every = kwargs.pop('callback_every', 10)
    ...
    def train(self):
        ...
        for t in range(num_iterations):
            ...
            # Calling callback
            if self.callback is not None and self.verbose and t % self.callback_every == 0:
                bleuscores = self.callback(self.model)
                if bleuscores['val'] > self.best_score:
                    self.best_score = bleuscores['val']
                    self.best_model = copy.deepcopy(self.model)
                    self.best_iterations = t
